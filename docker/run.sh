...
showInstructions()
{
    ...
    declare -ga services=(
        [1]="Set up new Network"
        [2]="Show current Network"
        [3]="Install project"
...
setUpNetwork()
{
    ...
    while read -p "Type the network name [example: jumia]: " networkname
    do
        printf "\n"
...
installProject()
{
    ...
    bash ${DOCKER_PROJECT_PATH}/environments/project/install.sh
}
...
enterProject()
{
    ...
    bash ${DOCKER_PROJECT_PATH}/environments/project/enter.sh
}

uninstallProject()
{
    bash ${DOCKER_PROJECT_PATH}/environments/project/uninstall.sh
}
